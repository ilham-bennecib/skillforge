...
#             cursor.execute(
#                 """
#                 INSERT INTO company ("lastName", "firstName", "email", "phone", "directory", "roleId")
#                 VALUES (%s, %s, %s, %s, %s, %s)
#                 RETURNING id
...
#                 cursor.execute("DELETE FROM company WHERE id = %s", [user_id])
...
#         try:
#             with self.connection.cursor() as cursor:
#                 #verifiactaion de l'existance du companie
#                 cursor.execute("SELECT * FROM company WHERE id = %s", [user_id])
#                 companie = cursor.fetchone()
#                 if companie is None:
